...
class FormKomplainUsulan extends RestController {
    ...
    public function index_post(){
        ...
        if(!empty($param['idUser']) && !empty($param['idMapping'])){
            ...
            if($user != null && $mapping != null){
                ...
                $idKomplain     = 'KOMPLAIN_'.substr(md5(time()."komplain"), 0, 14);
                ...
                $storeKomplain['ID_KOMPLAIN']       = $idKomplain;
                $storeKomplain['ID_TRANS']          = $idTrans;
                $this->db->insert('FORM_KOMPLAIN', $storeKomplain);

                foreach($param['detKomplain'] as $item){
                    $storeDetKomplain['ID_KOMPLAIN']     = $idKomplain;
                    $storeDetKomplain['NAMA_KOMPLAIN']   = $user[0]->NAMA_USERS; 
                    $storeDetKomplain['DIV_KOMPLAIN']    = $user[0]->DIV_USERS; 
                    $storeDetKomplain['KOMPLAIN_USULAN'] = $item['komplainUsul'];
                    $this->db->insert('DETAIL_KOMPLAIN', $storeDetKomplain);
                }
