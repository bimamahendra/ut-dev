<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>FORM PERMINTAAN EXTENSION DAN AKSES</title>

    <style>
        html,
        body {
            font-family: Arial, Helvetica, sans-serif;
            font-size: 14px;
        }

        .w-100 {
            min-width: 100%;
        }

        .w-50 {
            min-width: 50%;
            max-width: 50%;
        }

        .table-center {
            margin-right: auto;
            margin-left: auto;
        }

        .table-layout-fixed {
            table-layout: fixed;
        }

        .p-min {
            padding: .2em .3em;
        }

        .p-med {
            padding: .8em .3em;
        }

        .pl-min {
            padding-left: .3em;
        }

        .pl-med {
            padding-left: .6em;
        }

        .pt-min {
            padding-top: .5em;
        }

        .pb-min {
            padding-bottom: .4em;
        }

        .valign-middle {
            vertical-align: middle;
        }

        .p-max {
            padding: 2em;
        }

        .mb-max {
            margin-bottom: 3em;
        }

        .mb-min {
            margin-bottom: .7em;
        }

        .border-collapse {
            border-collapse: collapse;
        }

        .border-1 {
            border: 1px solid black;
        }

        .border-2 {
            border: 2px solid black;
        }

        .border-right {
            border-right: 1px solid black;
        }

        .border-left-bold {
            border-left: 3px solid black;
        }

        .border-bottom {
            border-bottom: 1px solid black;
        }

        .text-title {
            font-size: 24px;
        }

        .text-regular {
            font-size: 14px;
        }

        .text-regular-sm {
            font-size: 12px;
        }

        .text-regular-xs {
            font-size: 10px;
        }

        .font-weight-regular {
            font-weight: 400;
        }

        .font-weight-bold {
            font-weight: 800;
        }

        .text-align-left {
            text-align: left;
        }

        .text-align-right {
            text-align: right;
        }

        .text-align-center {
            text-align: center;
        }

        .text-align-justify {
            text-align: justify;
        }

        .text-vertical {
            writing-mode: vertical-rl;
        }

        .watermark::before {
            position: absolute;
            top: 35%;
            left: 10%;
            content: "General Affairs Department";
            z-index: 99;
            font-size: 80px;
            transform: rotate(-30deg);
            color: rgba(0, 0, 0, .15);
            text-align: center;
            line-height: 1.8em;
        }

        .wrapper-page {
            page-break-after: always;
        }

        .wrapper-page:last-child {
            page-break-after: avoid;
        }

        .bg-grey {
            background-color: #C0C0C0;
        }

        .bg-soft-blue {
            background-color: #CCFFFF;
        }

        .absolute {
            position: absolute;
        }

        .pos-right {
            right: 0;
        }

        .pos-left {
            left: 0;
        }
    </style>

</head>

<body>
    <div class="wrapper-page">
        <div class="watermark"></div>
        <table class="border-2 border-collapse w-100 valign-middle mb-min">
            <tr>
                <td class="text-regular-sm font-family-arial text-align-center p-med border-right">
                    <img class="valign-middle" src="data:image/png;base64," height="25px" width="25px">
                    &nbsp;<strong class="valign-middle">UNITED TRACTORS</strong>
                </td>
                <td class="text-align-center border-1" colspan="2"><strong>FORM PERMINTAAN EXTENSION DAN AKSES</strong></td>
                <td class="text-align-center border-bottom"><strong>INTEGRASI SISTEM</strong></td>
            </tr>
            <tr>
                <td class="text-regular-sm text-align-center border-right" rowspan="3">Jl. Raya Bekasi KM 22 Cakung <br> Jakarta Timur</td>
                <td class="text-regular-sm pl-min pt-min" style="width: 15%;">
                    <strong>Nomor Dokumen</strong>
                </td>
                <td class="text-regular-sm border-right pt-min" style="width: 30%;">: FORM 012/PROS-MFP-MLK3-014</td>
                <td class="text-regular-sm text-align-center pt-min pb-min" rowspan="3">
                    <strong>
                        ISO 9001 : 2008 ; 1SO <br>
                        14001 : 2004 ; OHSAS <br>
                        18001 : 2007 &amp; SMK3
                    </strong>
                </td>
            </tr>
            <tr>
                <td class="text-regular-sm pl-min" style="width: 15%;">
                    <strong>No. Revisi</strong>
                </td>
                <td class="text-regular-sm border-right">: 1</td>
            </tr>
            <tr>
                <td class="text-regular-sm pl-min pb-min" style="width: 15%;">
                    <strong>Hal</strong>
                </td>
                <td class="text-regular-sm pb-min border-right">: ... Dari ...</td>
            </tr>
        </table>
        <table class="mb-min w-100">
            <tr>
                <td width="20%">Divisi / Dept / Cabang / Site</td>
                <td>: General Affairs</td>
            </tr>
            <tr>
                <td>Tanggal</td>
                <td>: 18 Maret 2021</td>
            </tr>
        </table>
        <table class="border-1 border-collapse valign-middle text-align-center w-100 table-layout-fixed mb-min">
            <tr>
                <th class="border-1 text-regular-sm" rowspan="3" width="3%">No</th>
                <th class="border-1 text-regular-sm" rowspan="3" width="15%">Permintaan Atas Nama</th>
                <th class="border-1 text-regular-sm" rowspan="3" width="8%">NRP</th>
                <th class="border-1 text-regular-sm" rowspan="3" width="11%">Jabatan</th>
                <th class="border-1 text-regular-sm" rowspan="3" width="8%">Nomor Extension</th>
                <th class="border-1 text-regular-sm" colspan="2" rowspan="2">Jenis Permintaan / Perubahan Fasilitas Akses*</th>
                <th class="border-1 text-regular-sm border-left-bold" colspan="8">Alasan Permintaan</th>
            </tr>
            <tr>
                <th class="border-1 text-regular-sm border-left-bold text-align-left pl-min" colspan="8">Contact to :</th>
            </tr>
            <tr>
                <th class="border-1 text-regular-sm" width="8%">Akses Existing</th>
                <th class="border-1 text-regular-sm" width="8%">Akses Baru</th>
                <th width="5%" class="border-1 text-regular-xs p-min border-left-bold">Principle</th>
                <th width="5%" class="border-1 text-regular-xs p-min">UTHI</th>
                <th width="5%" class="border-1 text-regular-xs p-min">Cabang / Site</th>
                <th width="5%" class="border-1 text-regular-xs p-min">Partner</th>
                <th width="5%" class="border-1 text-regular-xs p-min">Customer</th>
                <th width="5%" class="border-1 text-regular-xs p-min">Vendor</th>
                <th width="5%" class="border-1 text-regular-xs p-min">Subcont</th>
                <th width="5%" class="border-1 text-regular-xs p-min">Hotel</th>
            </tr>
            <tr>
                <td class="border-1">1</td>
                <td class="border-1 text-align-left pl-min">Giri</td>
                <td class="border-1 text-align-left pl-min">181111011</td>
                <td class="border-1 text-align-left pl-min">Section Head</td>
                <td class="border-1 text-align-left pl-min">12</td>
                <td class="border-1 text-align-left pl-min">I</td>
                <td class="border-1 text-align-left pl-min">II</td>
                <td class="border-1 border-left-bold">V</td>
                <td class="border-1"></td>
                <td class="border-1"></td>
                <td class="border-1"></td>
                <td class="border-1"></td>
                <td class="border-1"></td>
                <td class="border-1"></td>
                <td class="border-1"></td>
            </tr>
            <tr>
                <td class="border-1">2</td>
                <td class="border-1 text-align-left pl-min">Ilham</td>
                <td class="border-1 text-align-left pl-min">181111063</td>
                <td class="border-1 text-align-left pl-min">Section Head</td>
                <td class="border-1 text-align-left pl-min">123</td>
                <td class="border-1 text-align-left pl-min">I</td>
                <td class="border-1 text-align-left pl-min">II</td>
                <td class="border-1 border-left-bold"></td>
                <td class="border-1"></td>
                <td class="border-1">V</td>
                <td class="border-1"></td>
                <td class="border-1"></td>
                <td class="border-1"></td>
                <td class="border-1"></td>
                <td class="border-1"></td>
            </tr>
            <tr>
                <td class="border-1">3</td>
                <td class="border-1 text-align-left pl-min">Zidan</td>
                <td class="border-1 text-align-left pl-min">181111067</td>
                <td class="border-1 text-align-left pl-min">Section Head</td>
                <td class="border-1 text-align-left pl-min">1234</td>
                <td class="border-1 text-align-left pl-min">I</td>
                <td class="border-1 text-align-left pl-min">II</td>
                <td class="border-1 border-left-bold"></td>
                <td class="border-1"></td>
                <td class="border-1"></td>
                <td class="border-1"></td>
                <td class="border-1"></td>
                <td class="border-1">V</td>
                <td class="border-1"></td>
                <td class="border-1"></td>
            </tr>
            <tr>
                <td class="border-1">4</td>
                <td class="border-1 text-align-left pl-min">Gellen</td>
                <td class="border-1 text-align-left pl-min">181111023</td>
                <td class="border-1 text-align-left pl-min">Section Head</td>
                <td class="border-1 text-align-left pl-min">12345</td>
                <td class="border-1 text-align-left pl-min">I</td>
                <td class="border-1 text-align-left pl-min">II</td>
                <td class="border-1 border-left-bold"></td>
                <td class="border-1"></td>
                <td class="border-1"></td>
                <td class="border-1"></td>
                <td class="border-1">V</td>
                <td class="border-1"></td>
                <td class="border-1"></td>
                <td class="border-1"></td>
            </tr>
        </table>
        <table class="border-1 border-collapse absolute pos-right table-layout-fixed" style="width: 600px;">
            <tr>
                <th class="text-align-center border-1" width="33.33%">Staff / User</th>
                <th class="text-align-center border-1" width="33.33%">Section Head</th>
                <th class="text-align-center border-1" width="33.33%">Dept Head</th>
            </tr>
            <tr>
                <td class="text-align-center border-1 p-min"><img src="https://via.placeholder.com/100" alt=""></td>
                <td class="text-align-center border-1 p-min"><img src="https://via.placeholder.com/100" alt=""></td>
                <td class="text-align-center border-1 p-min"><img src="https://via.placeholder.com/100" alt=""></td>
            </tr>
            <tr>
                <td class="text-align-center border-1">( Nama Terang )</td>
                <td class="text-align-center border-1">( Nama Terang )</td>
                <td class="text-align-center border-1">( Nama Terang )</td>
            </tr>
        </table>
        <p>
            * FASILITAS AKSES <br>
            I : SLI, SLJJ, HP, LOKAL, INTERN, CABANG <br>
            II : SLJJ, HP, LOKAL, INTERN, CABANG <br>
            III : HP, LOKAL, INTERN, CABANG <br>
            IV : LOKAL, INTERN, CABANG <br>
            V : INTERN CABANG <br>
        </p>
    </div>
</body>

</html>