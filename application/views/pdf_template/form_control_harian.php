<!DOCTYPE html>
<html lang="en">

<head>
	<meta charset="UTF-8">
	<meta name="viewport" content="width=device-width, initial-scale=1.0">
	<title>FORM KONTROL HARIAN</title>
	<style>
		.watermark::before {
			position: absolute;
			top: 35%;
			left: 10%;
			content: "General Affairs Department";
			z-index: 99;
			font-size: 80px;
			transform: rotate(-30deg);
			color: rgba(0, 0, 0, .2);
		}

		.tg {
			border-style: solid;
			border-width: 2px;
			border-collapse: collapse;
			border-spacing: 0;
		}

		.tg td {
			font-family: Arial, sans-serif;
			font-size: 14px;
			overflow: hidden;
			word-break: normal;
		}

		.tg th {
			border-color: black;
			border-style: solid;
			border-width: 2px;
			font-family: Arial, sans-serif;
			font-size: 14px;
			font-weight: normal;
			overflow: hidden;
			padding: 10px 5px;
			word-break: normal;
		}

		.tg .tg-w2dt {
			font-size: 12px;
			text-align: center;
			vertical-align: middle;
			border-color: black;
			border-style: solid;
			border-width: 2px;
			width: 28%;
		}

		.tg .tg-z9od {
			font-size: 12px;
			text-align: left;
			vertical-align: center;
			padding-left: 5px;
			padding-top: 5px
		}

		.tg .tg-z9ad {
			font-size: 12px;
			text-align: left;
			vertical-align: center;
			padding-left: 5px
		}

		.tg .tg-z9cd {
			font-size: 12px;
			text-align: left;
			vertical-align: center;
			padding-left: 5px;
			padding-bottom: 5px
		}

		.tg .tg-xsvg {
			font-size: 12px;
			font-weight: bold;
			text-align: justify;
			padding-left: 25px;
			border-color: black;
			border-style: solid;
			border-width: 2px;
		}

		.tg .tg-ir4y {
			font-size: 12px;
			font-weight: bold;
			text-align: center;
		}

		.tg .tg-k27y {
			font-size: 12px;
			font-weight: bold;
			text-align: center;
			vertical-align: middle
		}

		.tp {
			border: 1px solid black;
			;
			border-collapse: collapse;
			width: 100%;
			text-align: left;
			font-size: 13px;
		}

		.tp th {
			border: 1px solid black;
			padding: 3px;
			border-collapse: collapse;
			width: 100%;
			text-align: left;
		}

		.tp td {
			padding: 3px;
			border: 1px solid black;
			border-collapse: collapse;
			width: 100%;
			text-align: left;
		}

		.tj {
			border: 1px solid black;
			border-collapse: collapse;
			width: 100%;
			text-align: center;
			font-size: 14px;
		}

		.tj th {
			border: 1px solid black;
			border-collapse: collapse;
			text-align: center;
		}

		.tj td {
			border: 1px solid black;
			border-collapse: collapse;
			text-align: center;
		}

		.thd {
			border: none;
			border-collapse: collapse;
			text-align: center;
			width: 25%;
			font-size: 14px;
			float: right;
		}

		.thd::after {
			content: "";
			clear: both;
		}

		.thd-tha {
			border: 1px solid black;
			border-collapse: collapse;
		}

		.thd-td2 {
			border-right: 1px solid black;
		}

		.tg .tg-5rbv {
			border: 1px solid black;
			font-size: 14px;
			border-color: black;
			font-weight: bold;
			text-align: center;
			vertical-align: middle
		}

		.tg .tg-8jxv {
			border: 1px solid black;
			font-size: 14px;
			font-weight: bold;
			vertical-align: top;
			text-align: center;

		}

		.tg .tg-18ri {
			border: 1px solid black;
			font-family: Arial, Helvetica, sans-serif !important;
			font-size: 14px;
			font-weight: bold;
			text-align: center;
			vertical-align: top
		}

		.tg .tg-5r9a {
			border: 1px solid black;
			padding-top: 8px;
			padding-bottom: 8px;
			font-size: 14px;
			vertical-align: top
		}

		.thc {
			border: 1px solid black;
			border-collapse: collapse;
			text-align: center;
			width: 100%;
			font-size: 14px;
			position: absolute;
			right: 0;
		}

		.thc-tha {
			border: 1px solid black;
			border-collapse: collapse;
		}

		.thc-td2 {
			border: 1px solid black;
		}

		.text-align-center {
			text-align: center;
		}

		.padding-left {
			padding-left: 6px;
		}
	</style>
</head>

<body style="padding-left:0.5cm; font-family:Arial, sans-serif;">
	<div class="watermark"></div>
	<div>
		<table class="tg" width="100%;">
			<thead>
				<tr>
					<th class="tg-ir4y">
						<img src="data:image/png;base64," height="25px" width="25px">
						UNITED TRACTORS
					</th>
					<th class="tg-k27y" colspan="2" rowspan="2">FORM KONTROL HARIAN</th>
					<th class="tg-k27y" rowspan="2">INTEGRASI SISTEM</th>
				</tr>
				<tr>
					<td class="tg-w2dt" rowspan="4">Jl. Raya Bekasi Km. 22 Cakung<br>
						Jakarta Timur</td>
				</tr>
			</thead>
			<tbody>
				<tr>
					<td class="tg-z9od" width="13%">No. Dokumen</td>
					<td class="tg-z9od" width="33%"> : FORM 006/PROS-MFP-MLK3-013 </td>
					<td class="tg-xsvg" rowspan="3">ISO 9001 : 2008 ; 1SO <br>
						14001 : 2004 ; OHSAS <br>
						18001 : 2007 &amp; SMK3
				</tr>
				<tr>
					<td class="tg-z9ad">Revisi</td>
					<td class="tg-z9ad">: 1</td>
				</tr>
				<tr>
					<td class="tg-z9cd">Hal</td>
					<td class="tg-z9cd">: … Dari : …</td>
				</tr>
			</tbody>
		</table>
	</div>
	<div style="text-align:center">
		<h4>FORM KONTROL HARIAN</h4>
	</div>
	<br>
	<div>
		<table style="width:80%; text-align:left;">
			<tr>
				<td style="width: 15%">Kedatangan Jam</td>
				<td style="width: 5%">:</td>
				<td>
					<?php
						$dtime = new DateTime($list[0]->JAM_CONTROL);
						echo $dtime->format("H:i:s");
					?>
				</td>
			</tr>
		</table>
	</div>
	<br>
	<div>
		<table class="tg" width="100%" style="border-color: black;">
			<thead>
				<tr>
					<th class="tg-5rbv" rowspan="2" width="5%">No</th>
					<th class="tg-5rbv" rowspan="2">Pemeriksaan</th>
					<th class="tg-5rbv" colspan="4">Kondisi</th>
					<th class="tg-5rbv" rowspan="2">Keterangan</th>
				</tr>
				<tr>
					<td class="tg-18ri">Baik Sekali</td>
					<td class="tg-8jxv">Baik</td>
					<td class="tg-8jxv">Cukup</td>
					<td class="tg-8jxv">Kurang</td>
				</tr>
			</thead>
			<tbody>
				<tr>
					<td class="tg-5r9a text-align-center">1.</td>
					<td class="tg-5r9a padding-left">Kondisi Karyawan</td>
					<?php
					$pemeriksaan = explode(';', $list[0]->KONDISI_CONTROL);
					echo '
							<td class="tg-5r9a text-align-center">' . ($pemeriksaan[0] == 1 ? 'v' : '') . '</td>
							<td class="tg-5r9a text-align-center">' . ($pemeriksaan[0] == 2 ? 'v' : '') . '</td>					
							<td class="tg-5r9a text-align-center">' . ($pemeriksaan[0] == 3 ? 'v' : '') . '</td>
							<td class="tg-5r9a text-align-center">' . ($pemeriksaan[0] == 4 ? 'v' : '') . '</td>
							<td class="tg-5r9a text-align-center">' . $pemeriksaan[1] . '</td>
						';
					?>
				</tr>
				<tr>
					<td class="tg-5r9a text-align-center">2.</td>
					<td class="tg-5r9a padding-left">Kendaraan</td>
					<?php
					$pemeriksaan = explode(';', $list[0]->KENDARAAN_CONTROL);
					echo '
							<td class="tg-5r9a text-align-center">' . ($pemeriksaan[0] == 1 ? 'v' : '') . '</td>
							<td class="tg-5r9a text-align-center">' . ($pemeriksaan[0] == 2 ? 'v' : '') . '</td>					
							<td class="tg-5r9a text-align-center">' . ($pemeriksaan[0] == 3 ? 'v' : '') . '</td>
							<td class="tg-5r9a text-align-center">' . ($pemeriksaan[0] == 4 ? 'v' : '') . '</td>
							<td class="tg-5r9a text-align-center">' . $pemeriksaan[1] . '</td>
						';
					?>
				</tr>
				<tr>
					<td class="tg-5r9a text-align-center">3. </td>
					<td class="tg-5r9a padding-left">Penyajian Makanan</td>
					<?php
					$pemeriksaan = explode(';', $list[0]->MAKANAN_CONTROL);
					echo '
							<td class="tg-5r9a text-align-center">' . ($pemeriksaan[0] == 1 ? 'v' : '') . '</td>
							<td class="tg-5r9a text-align-center">' . ($pemeriksaan[0] == 2 ? 'v' : '') . '</td>					
							<td class="tg-5r9a text-align-center">' . ($pemeriksaan[0] == 3 ? 'v' : '') . '</td>
							<td class="tg-5r9a text-align-center">' . ($pemeriksaan[0] == 4 ? 'v' : '') . '</td>
							<td class="tg-5r9a text-align-center">' . $pemeriksaan[1] . '</td>
						';
					?>
				</tr>
				<tr>
					<td class="tg-5r9a text-align-center">4.</td>
					<td class="tg-5r9a padding-left">Jumlah Pesanan</td>
					<?php
					$pemeriksaan = explode(';', $list[0]->PESANAN_CONTROL);
					echo '
							<td class="tg-5r9a text-align-center">' . ($pemeriksaan[0] == 1 ? 'v' : '') . '</td>
							<td class="tg-5r9a text-align-center">' . ($pemeriksaan[0] == 2 ? 'v' : '') . '</td>					
							<td class="tg-5r9a text-align-center">' . ($pemeriksaan[0] == 3 ? 'v' : '') . '</td>
							<td class="tg-5r9a text-align-center">' . ($pemeriksaan[0] == 4 ? 'v' : '') . '</td>
							<td class="tg-5r9a text-align-center">' . $pemeriksaan[1] . '</td>
						';
					?>
				</tr>
				<tr>
					<td class="tg-5r9a text-align-center">5.</td>
					<td class="tg-5r9a padding-left">Sample yang ditempatkan di poliklinik</td>
					<?php
					$pemeriksaan = explode(';', $list[0]->SAMPLE_CONTROL);
					echo '
							<td class="tg-5r9a text-align-center">' . ($pemeriksaan[0] == 1 ? 'v' : '') . '</td>
							<td class="tg-5r9a text-align-center">' . ($pemeriksaan[0] == 2 ? 'v' : '') . '</td>					
							<td class="tg-5r9a text-align-center">' . ($pemeriksaan[0] == 3 ? 'v' : '') . '</td>
							<td class="tg-5r9a text-align-center">' . ($pemeriksaan[0] == 4 ? 'v' : '') . '</td>
							<td class="tg-5r9a text-align-center">' . $pemeriksaan[1] . '</td>
						';
					?>
				</tr>
				<tr>
					<td class="tg-5r9a text-align-center">6.</td>
					<td class="tg-5r9a padding-left">Kebersihan Penyajian</td>
					<?php
					$pemeriksaan = explode(';', $list[0]->KEBERSIHAN_CONTROL);
					echo '
							<td class="tg-5r9a text-align-center">' . ($pemeriksaan[0] == 1 ? 'v' : '') . '</td>
							<td class="tg-5r9a text-align-center">' . ($pemeriksaan[0] == 2 ? 'v' : '') . '</td>					
							<td class="tg-5r9a text-align-center">' . ($pemeriksaan[0] == 3 ? 'v' : '') . '</td>
							<td class="tg-5r9a text-align-center">' . ($pemeriksaan[0] == 4 ? 'v' : '') . '</td>
							<td class="tg-5r9a text-align-center">' . $pemeriksaan[1] . '</td>
						';
					?>
				</tr>
			</tbody>
		</table>
	</div>
	<br>
	<div>
		<table class="thc" style="width: 450px;table-layout: fixed;">
			<tr>
				<td class="thc-tha" style="min-width: 50%;max-width: 50%;"><b>User/Staff</b></td>
				<td class="thc-tha" style="min-width: 50%;max-width: 50%;"><b>Section Head</b></td>
			</tr>
			<tr style="height:300px; min-height:300px;">
				<td class="thc-td2">
					<img src="<?= $user->PATH_TTD ?>" width="100px" height="100px" />
				</td>
				<td class="thc-td2">
					<?php
					if ($approvals[0]->ROLE_APP == "Section Head" && $approvals[0]->ISAPPROVE_APP == "1") {
						echo '
								<img src="' . $approvals[0]->PATH_TTD . '" width="100px" height="100px" />
							';
					}
					?>
				</td>
			</tr>
			<tr>
				<td class="thc-td2">( <?= $user->NAMA_USERS ?> )</td>
				<?php
				if ($approvals[0]->ROLE_APP == "Section Head" && $approvals[0]->ISAPPROVE_APP == "1") {
					echo '
							<td "thc-td2">( ' . $approvals[0]->NAMA_USERS . ' )</td>
						';
				} else {
					echo '
							<td "thc-td2">(................................................)</td>
						';
				}
				?>
			</tr>
		</table>
	</div>
</body>

</html>