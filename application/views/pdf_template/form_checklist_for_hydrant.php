<!DOCTYPE html>
<html lang="en">

<head>
	<meta charset="UTF-8">
	<meta name="viewport" content="width=device-width, initial-scale=1.0">
	<title>FORM INSPECTION CHECK LIST for HYDRANT</title>
	<style>
		.watermark::before {
			position: absolute;
			top: 35%;
			left: 10%;
			content: "General Affairs Department";
			z-index: 99;
			font-size: 80px;
			transform: rotate(-30deg);
			color: rgba(0, 0, 0, .15);
			text-align: center;
		}

		.valign-middle {
			vertical-align: middle;
		}

		.tg {
			border-style: solid;
			border-width: 2px;
			border-collapse: collapse;
			border-spacing: 0;
		}

		.tg td {
			font-family: Arial, sans-serif;
			font-size: 14px;
			overflow: hidden;
			word-break: normal;
		}

		.tg th {
			border-color: black;
			border-style: solid;
			border-width: 2px;
			font-family: Arial, sans-serif;
			font-size: 14px;
			font-weight: normal;
			overflow: hidden;
			padding: 10px 5px;
			word-break: normal;
		}

		.tg .tg-w2dt {
			font-size: 12px;
			text-align: center;
			vertical-align: middle;
			border-color: black;
			border-style: solid;
			border-width: 2px;
			width: 28%;
		}

		.tg .tg-z9od {
			font-size: 12px;
			text-align: left;
			vertical-align: center;
			padding-left: 5px;
			padding-top: 5px
		}

		.tg .tg-z9ad {
			font-size: 12px;
			text-align: left;
			vertical-align: center;
			padding-left: 5px
		}

		.tg .tg-z9cd {
			font-size: 12px;
			text-align: left;
			vertical-align: center;
			padding-left: 5px;
			padding-bottom: 5px
		}

		.tg .tg-xsvg {
			font-size: 12px;
			font-weight: bold;
			text-align: justify;
			padding-left: 25px;
			border-color: black;
			border-style: solid;
			border-width: 2px;
		}

		.tg .tg-ir4y {
			font-size: 12px;
			font-weight: bold;
			text-align: center;
		}

		.tg .tg-k27y {
			font-size: 12px;
			font-weight: bold;
			text-align: center;
			vertical-align: middle
		}

		.tp {
			border: 1px solid black;
			;
			border-collapse: collapse;
			width: 100%;
			text-align: left;
			font-size: 13px;
		}

		.tp th {
			border: 1px solid black;
			padding: 3px;
			border-collapse: collapse;
			width: 100%;
			text-align: left;
		}

		.tp td {
			padding: 3px;
			border: 1px solid black;
			border-collapse: collapse;
			width: 100%;
			text-align: left;
		}

		.tj {
			border: 1px solid black;
			border-collapse: collapse;
			width: 100%;
			text-align: center;
			font-size: 14px;
		}

		.tj th {
			border: 1px solid black;
			border-collapse: collapse;
			text-align: center;
		}

		.tj td {
			border: 1px solid black;
			border-collapse: collapse;
			text-align: center;
		}

		.thd {
			border: 1px solid black;
			border-collapse: collapse;
			text-align: center;
			width: 66, 6%;
			font-size: 12px;
		}

		.thd-tha {
			border: 1px solid black;
			border-collapse: collapse;
		}

		.thd-td2 {
			border-right: 1px solid black;
		}

		.tg .tg-5r9a {
			border: 1px solid black;
			padding-top: 2px;
			padding-bottom: 2px;
			padding-left: 6px;
			font-size: 9px;
			text-align: left;
			vertical-align: top
		}

		.tg .tg-5rbv {
			border: 1px solid black;
			font-size: 9px;
			border-color: black;
			font-weight: bold;
			text-align: center;
			vertical-align: middle
		}
	</style>
</head>

<body style="padding-left:0.5cm; font-family:Arial, sans-serif;">
	<div class="watermark"></div>
	<div>
		<table class="tg" width="100%;">
			<thead>
				<tr>
					<th class="tg-ir4y">
						<img class="valign-middle" src="data:image/png;base64," height="25px" width="25px">
						&nbsp;<strong class="valign-middle">UNITED TRACTORS</strong>
					</th>
					<th class="tg-k27y" colspan="2" rowspan="2">INSPECTION CHECK LIST for HYDRANT</th>
					<th class="tg-k27y" rowspan="2">INTEGRASI SISTEM</th>
				</tr>
				<tr>
					<td class="tg-w2dt" rowspan="4">Jl. Raya Bekasi Km. 22 Cakung<br>
						Jakarta Timur</td>
				</tr>
			</thead>
			<tbody>
				<tr>
					<td class="tg-z9od" width="13%">No. Dokumen</td>
					<td class="tg-z9od" width="33%"> : <?= $noDoc ?></td>
					<td class="tg-xsvg" rowspan="3">ISO 9001 : 2008 ; 1SO<br>
						14001 : 2004 ; OHSAS<br>
						18001 : 2007 &amp; SMK3</td>
				</tr>
				<tr>
					<td class="tg-z9ad">Revisi</td>
					<td class="tg-z9ad">: 0</td>
				</tr>
				<tr>
					<td class="tg-z9cd">Hal</td>
					<td class="tg-z9cd">: … Dari : …</td>
				</tr>
			</tbody>
		</table>
	</div>
	<div style="text-align:center">
		<h5>WEEKLY REPORT HYDRANT</h5>
	</div>
	<div>
		<table style="width:80%; text-align:left; font-size: 8px;">
			<tr>
				<td style="width: 25%" colspan="2"><b>Seksi Maintenance</b></td>
				<td> &nbsp; <b>GA Departement</b></td>
			</tr>
			<tr>
				<td style="width: 3%"><b>Lokasi</b></td>
				<td style="width: 2%"><b>:</b></td>
				<td>
					<?= $list[0]->LOKASI_HYDRANT; ?>
				</td>
			</tr>
			<tr>
				<td style="width: 3%"><b>Tgl</b></td>
				<td style="width: 2%"><b>:</b></td>
				<td>
					<?php
					$date = date_create($list[0]->TGL_HYDRANT);
					echo '
							' . date_format($date, 'd - m - Y') . '
						';
					?>
				</td>
			</tr>
		</table>
	</div>
	<div>
		<table class="tg" width="100%" style="border-color: black;">
			<thead>
				<tr>
					<th class="tg-5rbv" width="5%">No</th>
					<th class="tg-5rbv">ITEM CHECK</th>
					<th class="tg-5rbv">STANDARD</th>
					<th class="tg-5rbv" width="10%">V</th>
					<th class="tg-5rbv" width="10%">P</th>
				</tr>
			</thead>
			<tbody>
				<tr>
					<td class="tg-5r9a" style="text-align:center"><b>1.</b></td>
					<td class="tg-5r9a"><b>SYSTEM PEMIPAAN : </b></td>
					<td class="tg-5r9a"></td>
					<td class="tg-5r9a"></td>
					<td class="tg-5r9a"></td>
				</tr>
				<?php
				$status = explode(';', $list[0]->SISTEM_PEMIPAAN);
				?>
				<tr>
					<td class="tg-5r9a"></td>
					<td class="tg-5r9a">a. Kebocoran Pipa</td>
					<td class="tg-5r9a">Tidak ada kebocoran</td>
					<td class="tg-5r9a" style="text-align:center"><?= ($status[0] == '1' ? 'V' : '') ?></td>
					<td class="tg-5r9a" style="text-align:center"><?= ($status[0] == '0' ? 'V' : '') ?></td>
				</tr>
				<tr>
					<td class="tg-5r9a"></td>
					<td class="tg-5r9a">b. Flange / Packing</td>
					<td class="tg-5r9a">Tidak ada kebocoran</td>
					<td class="tg-5r9a" style="text-align:center"><?= ($status[1] == '1' ? 'V' : '') ?></td>
					<td class="tg-5r9a" style="text-align:center"><?= ($status[1] == '0' ? 'V' : '') ?></td>
				</tr>
				<tr>
					<td class="tg-5r9a"></td>
					<td class="tg-5r9a">c. Posisi Valve</td>
					<td class="tg-5r9a">Sesuai dengan posisi (OPEN/SHUT)</td>
					<td class="tg-5r9a" style="text-align:center"><?= ($status[2] == '1' ? 'V' : '') ?></td>
					<td class="tg-5r9a" style="text-align:center"><?= ($status[2] == '0' ? 'V' : '') ?></td>
				</tr>
				<?php
				$status = explode(';', $list[0]->JOCKEY_PUMP);
				?>
				<tr>
					<td class="tg-5r9a" style="text-align:center"><b>2.</b></td>
					<td class="tg-5r9a"><b>JOCKEY PUMP : </b></td>
					<td class="tg-5r9a"></td>
					<td class="tg-5r9a"></td>
					<td class="tg-5r9a"></td>
				</tr>
				<tr>
					<td class="tg-5r9a"></td>
					<td class="tg-5r9a">a. Gate Valve</td>
					<td class="tg-5r9a">Air pancingan selalu pada posisi 'OPEN'</td>
					<td class="tg-5r9a" style="text-align:center"><?= ($status[0] == '1' ? 'V' : '') ?></td>
					<td class="tg-5r9a" style="text-align:center"><?= ($status[0] == '0' ? 'V' : '') ?></td>
				</tr>
				<tr>
					<td class="tg-5r9a"></td>
					<td class="tg-5r9a">b. Arah Kipas Pompa</td>
					<td class="tg-5r9a">Putaran searah dengan arah panah</td>
					<td class="tg-5r9a" style="text-align:center"><?= ($status[1] == '1' ? 'V' : '') ?></td>
					<td class="tg-5r9a" style="text-align:center"><?= ($status[1] == '0' ? 'V' : '') ?></td>
				</tr>
				<tr>
					<td class="tg-5r9a"></td>
					<td class="tg-5r9a">c. Kabel Power Terminal</td>
					<td class="tg-5r9a">Posisi OFF pada waktu pemeriksaan</td>
					<td class="tg-5r9a" style="text-align:center"><?= ($status[2] == '1' ? 'V' : '') ?></td>
					<td class="tg-5r9a" style="text-align:center"><?= ($status[2] == '0' ? 'V' : '') ?></td>
				</tr>
				<tr>
					<td class="tg-5r9a"></td>
					<td class="tg-5r9a">d. Motor Pompa</td>
					<td class="tg-5r9a">Tidak ada kelainan bunyi motor</td>
					<td class="tg-5r9a" style="text-align:center"><?= ($status[3] == '1' ? 'V' : '') ?></td>
					<td class="tg-5r9a" style="text-align:center"><?= ($status[3] == '0' ? 'V' : '') ?></td>
				</tr>
				<?php
				$status = explode(';', $list[0]->ELECTRIC_PUMP);
				?>
				<tr>
					<td class="tg-5r9a" style="text-align:center"><b>3.</b></td>
					<td class="tg-5r9a"><b>ELECTRIC PUMP : </b></td>
					<td class="tg-5r9a"></td>
					<td class="tg-5r9a"></td>
					<td class="tg-5r9a"></td>
				</tr>
				<tr>
					<td class="tg-5r9a"></td>
					<td class="tg-5r9a">a. Gate Valve</td>
					<td class="tg-5r9a">Air pancingan selalu pada posisi 'OPEN'</td>
					<td class="tg-5r9a" style="text-align:center"><?= ($status[0] == '1' ? 'V' : '') ?></td>
					<td class="tg-5r9a" style="text-align:center"><?= ($status[0] == '0' ? 'V' : '') ?></td>
				</tr>
				<tr>
					<td class="tg-5r9a"></td>
					<td class="tg-5r9a">b. Arah Kipas Pompa</td>
					<td class="tg-5r9a">Putaran searah dengan arah panah</td>
					<td class="tg-5r9a" style="text-align:center"><?= ($status[1] == '1' ? 'V' : '') ?></td>
					<td class="tg-5r9a" style="text-align:center"><?= ($status[1] == '0' ? 'V' : '') ?></td>
				</tr>
				<tr>
					<td class="tg-5r9a"></td>
					<td class="tg-5r9a">c. Kabel Power Terminal</td>
					<td class="tg-5r9a">Posisi OFF pada waktu pemeriksaan</td>
					<td class="tg-5r9a" style="text-align:center"><?= ($status[2] == '1' ? 'V' : '') ?></td>
					<td class="tg-5r9a" style="text-align:center"><?= ($status[2] == '0' ? 'V' : '') ?></td>
				</tr>
				<tr>
					<td class="tg-5r9a"></td>
					<td class="tg-5r9a">d. Motor Pompa</td>
					<td class="tg-5r9a">Tidak ada kelainan bunyi motor</td>
					<td class="tg-5r9a" style="text-align:center"><?= ($status[3] == '1' ? 'V' : '') ?></td>
					<td class="tg-5r9a" style="text-align:center"><?= ($status[3] == '0' ? 'V' : '') ?></td>
				</tr>
				<?php
				$status = explode(';', $list[0]->DIESEL_PUMP);
				?>
				<tr>
					<td class="tg-5r9a" style="text-align:center"><b>4.</b></td>
					<td class="tg-5r9a"><b>DIESEL HYDRANT PUMP : </b></td>
					<td class="tg-5r9a"></td>
					<td class="tg-5r9a"></td>
					<td class="tg-5r9a"></td>
				</tr>
				<tr>
					<td class="tg-5r9a"></td>
					<td class="tg-5r9a">a. Gate Valve</td>
					<td class="tg-5r9a">Air pancingan selalu pada posisi 'OPEN'</td>
					<td class="tg-5r9a" style="text-align:center"><?= ($status[0] == '1' ? 'V' : '') ?></td>
					<td class="tg-5r9a" style="text-align:center"><?= ($status[0] == '0' ? 'V' : '') ?></td>
				</tr>
				<tr>
					<td class="tg-5r9a"></td>
					<td class="tg-5r9a">b. Air Radiator</td>
					<td class="tg-5r9a">Harus penuh (tidak boleh kosong)</td>
					<td class="tg-5r9a" style="text-align:center"><?= ($status[1] == '1' ? 'V' : '') ?></td>
					<td class="tg-5r9a" style="text-align:center"><?= ($status[1] == '0' ? 'V' : '') ?></td>
				</tr>
				<tr>
					<td class="tg-5r9a"></td>
					<td class="tg-5r9a">c. Oli Mesin Diesel</td>
					<td class="tg-5r9a">Pada level yang ditentukan</td>
					<td class="tg-5r9a" style="text-align:center"><?= ($status[2] == '1' ? 'V' : '') ?></td>
					<td class="tg-5r9a" style="text-align:center"><?= ($status[2] == '0' ? 'V' : '') ?></td>
				</tr>
				<tr>
					<td class="tg-5r9a"></td>
					<td class="tg-5r9a">d. Tali Kipas (Fan Belt)</td>
					<td class="tg-5r9a">Tidak retas dan kendor (ganti bila perlu)</td>
					<td class="tg-5r9a" style="text-align:center"><?= ($status[3] == '1' ? 'V' : '') ?></td>
					<td class="tg-5r9a" style="text-align:center"><?= ($status[3] == '0' ? 'V' : '') ?></td>
				</tr>
				<tr>
					<td class="tg-5r9a"></td>
					<td class="tg-5r9a">e. Air Battery</td>
					<td class="tg-5r9a">Pada level yang ditentukan</td>
					<td class="tg-5r9a" style="text-align:center"><?= ($status[4] == '1' ? 'V' : '') ?></td>
					<td class="tg-5r9a" style="text-align:center"><?= ($status[4] == '0' ? 'V' : '') ?></td>
				</tr>
				<tr>
					<td class="tg-5r9a"></td>
					<td class="tg-5r9a">f. Putaran Mesin (RPM)</td>
					<td class="tg-5r9a">Sesuai kebutuhan, setting (bila perlu)</td>
					<td class="tg-5r9a" style="text-align:center"><?= ($status[5] == '1' ? 'V' : '') ?></td>
					<td class="tg-5r9a" style="text-align:center"><?= ($status[5] == '0' ? 'V' : '') ?></td>
				</tr>
				<tr>
					<td class="tg-5r9a"></td>
					<td class="tg-5r9a">g. Test Running</td>
					<td class="tg-5r9a">Laksanakan 2 minggu sekali</td>
					<td class="tg-5r9a" style="text-align:center"><?= ($status[6] == '1' ? 'V' : '') ?></td>
					<td class="tg-5r9a" style="text-align:center"><?= ($status[6] == '0' ? 'V' : '') ?></td>
				</tr>
				<?php
				$status = explode(';', $list[0]->PANEL_HYDRANT);
				?>
				<tr>
					<td class="tg-5r9a" style="text-align:center"><b>5.</b></td>
					<td class="tg-5r9a"><b>PANEL HYDRANT : </b></td>
					<td class="tg-5r9a"></td>
					<td class="tg-5r9a"></td>
					<td class="tg-5r9a"></td>
				</tr>
				<tr>
					<td class="tg-5r9a"></td>
					<td class="tg-5r9a">a. Selector Switch</td>
					<td class="tg-5r9a">Harus selalu pada posisi 'AUTO'</td>
					<td class="tg-5r9a" style="text-align:center"><?= ($status[0] == '1' ? 'V' : '') ?></td>
					<td class="tg-5r9a" style="text-align:center"><?= ($status[0] == '0' ? 'V' : '') ?></td>
				</tr>
				<tr>
					<td class="tg-5r9a"></td>
					<td class="tg-5r9a">b. Lampu Indikator</td>
					<td class="tg-5r9a">Sesuai dengan Operation</td>
					<td class="tg-5r9a" style="text-align:center"><?= ($status[1] == '1' ? 'V' : '') ?></td>
					<td class="tg-5r9a" style="text-align:center"><?= ($status[1] == '0' ? 'V' : '') ?></td>
				</tr>
				<tr>
					<td class="tg-5r9a"></td>
					<td class="tg-5r9a">c. Volt meter</td>
					<td class="tg-5r9a">Putar selector switch u/ Volt meter</td>
					<td class="tg-5r9a" style="text-align:center"><?= ($status[2] == '1' ? 'V' : '') ?></td>
					<td class="tg-5r9a" style="text-align:center"><?= ($status[2] == '0' ? 'V' : '') ?></td>
				</tr>
				<tr>
					<td class="tg-5r9a"></td>
					<td class="tg-5r9a">d. Ampere meter</td>
					<td class="tg-5r9a">Seimbang dengan daya motor pompa masing-masing</td>
					<td class="tg-5r9a" style="text-align:center"><?= ($status[3] == '1' ? 'V' : '') ?></td>
					<td class="tg-5r9a" style="text-align:center"><?= ($status[3] == '0' ? 'V' : '') ?></td>
				</tr>
				<tr>
					<td class="tg-5r9a"></td>
					<td class="tg-5r9a">e. Konektor Panel</td>
					<td class="tg-5r9a">Pengencangan terminasi MCB, MCCB, kontaktor, relay, terminal bar (setiap 2 bulan sekali)</td>
					<td class="tg-5r9a" style="text-align:center"><?= ($status[4] == '1' ? 'V' : '') ?></td>
					<td class="tg-5r9a" style="text-align:center"><?= ($status[4] == '0' ? 'V' : '') ?></td>
				</tr>
			</tbody>
		</table>
	</div>
	<div>
		<table style="width:80%; text-align:left; font-size: 8px;">
			<tr>
				<td style="width: 2%"><b>V</b></td>
				<td style="width: 2%"><b>:</b></td>
				<td>
					OK
				</td>
			</tr>
			<tr>
				<td style="width: 2%"><b>P</b></td>
				<td style="width: 2%"><b>:</b></td>
				<td>
					Perbaikan
				</td>
			</tr>
		</table>
	</div>
	<div>
		<table style="width:80%; text-align:left; font-size: 12px;">
			<tr style="text-align:left;">
				<td style="width: 7%"><b>Catatan</b></td>
				<td style="width: 1%"><b>:</b></td>
				<td><?= $list[0]->CATATAN_HYDRANT ?></td>
			</tr>
		</table>
	</div>
	<br>
	<div>
		<table class="thd" align="right">
			<tr>
				<td class="thd-tha">Diketahui Oleh : </td>
				<td class="thd-tha">Disetujui Oleh : </td>
			</tr>
			<tr style="height:300px; min-height:300px;">
				<td class="thd-td2">
					<img src="<?= $user->PATH_TTD ?>" width="100px" height="100px" />
				</td>
				<td class="thd-td2">
					<?php
					if ($approvals[0]->ROLE_APP == "Section Head" && $approvals[0]->ISAPPROVE_APP == "1") {
						echo '
                                <img src="' . $approvals[0]->PATH_TTD . '" width="100px" height="100px" />
                            ';
					}
					?>
				</td>
			</tr>
			<tr>
				<td class="thd-tha"><b>PIC Maintenance</b></td>
				<td class="thd-tha"><b>Section Head</b></td>
			</tr>
			<tr>
				<td class="thd-td2"><?= $user->NAMA_USERS ?></td>
				<td class="thd-td2">
					<?php
					if ($approvals[0]->ROLE_APP == "Section Head" && $approvals[0]->ISAPPROVE_APP == "1") {
						echo '
								' . $approvals[0]->NAMA_USERS . '
							';
					} else {
						echo '
								...
							';
					}
					?>
				</td>
			</tr>
		</table>
	</div>
</body>

</html>